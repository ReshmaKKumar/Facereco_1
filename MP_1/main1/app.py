...
def student_register():
    ...
    if request.method == 'POST':
        ...
        password = request.form.get('password', '')
        raw_subject_ids = request.form.getlist('subject_id')

        # --- CORRECTION 1: Convert string Subject IDs to ObjectIds here ---
        subject_oids = []
        for sid_str in raw_subject_ids:
            try:
                # IMPORTANT: Convert to ObjectId here for consistency
                subject_oids.append(ObjectId(sid_str)) 
            except Exception:
                # If a subject ID is invalid, skip it
                print(f"Skipping invalid subject ID: {sid_str}")
                continue

        if not name or not roll_number or not email or not password or not subject_oids:
            flash('All fields are required and at least one valid subject must be selected.')
        ...
        student_data = {
            ...
            'password_hash': generate_password_hash(password),
            'face_encoding': None,
            'subjects': subject_oids, # <-- Store ObjectIds
            'created_at': datetime.utcnow()
        }
